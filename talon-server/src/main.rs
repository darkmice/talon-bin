 ...
 */
//! Talon default bundle server binary.
//!
//! Includes core + AI + LLM + Agent and serves HTTP/TCP/PgWire endpoints.
...
fn print_help() {
    println!("Talon — 默认扩展服务端 (core + ai + llm + agent)");
    println!();
    println!("用法: talon [选项]");
    println!();
    println!("选项:");
...
fn print_legal_banner() {
    ...
    println!("Flavor: default bundle (core + ai + llm + agent)");
